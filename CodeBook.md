...
Note: UnitsOfMeasure on Gyro and ACC points above indicate original measurements before normalization and bounding. The original data contained the normalized and bounded data.
...
(tBodyAcc-XYZ and tGravityAcc-XYZ) using another low pass Butterworth filter with a corner frequency of 0.3 Hz. Subsequently, the body linear acceleration and angular velocity were derived in time to obtain Jerk signals 
...
- 'train/X_train.txt': Training set. (Measurement Data)

- 'train/y_train.txt': Training labels. (Activity IDs)

- 'test/X_test.txt': Test set. (Measurement Data)
